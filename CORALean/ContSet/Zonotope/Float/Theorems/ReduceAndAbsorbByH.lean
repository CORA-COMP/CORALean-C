...
/-!
# `reduceAndAbsorbBy`'s generator count is the kept block plus the box
-/
...
@[simp] theorem Zonotope.reduceAndAbsorbBy_h (Z : Zonotope 𝕋 n)
    (part : Split (Fin Z.h) into (Fin q) ⊕ (Fin r)) :
    (Z.reduceAndAbsorbBy part).h = q + n := rfl
